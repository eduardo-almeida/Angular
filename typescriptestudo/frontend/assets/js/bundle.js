...
var SHOW_ERROR_MENSAGE = 'show-error-message';
...
function hideErrorMessages(form) {
    form
        ...
        .forEach(function (item) { return item.classList.remove(SHOW_ERROR_MENSAGE); });
}
...
function shouldSendForm(form) {
    var send = true;
    form.querySelectorAll('.' + SHOW_ERROR_MENSAGE).forEach(function () { return (send = false); });
    return send;
}
